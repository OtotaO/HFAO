...
| Q-18 | **Proactive anomaly surfacing (net-new scope beyond §15.2).** Today's §8.4 monitor engine fires only on user-pre-configured thresholds — the user has to know what to watch in advance. The Q-9 "automatically shares relevant insights strategically" framing implies a layer that surfaces patterns *the user didn't pre-define*: error-rate divergence across models, cost spike vs. last week's baseline, latency p99 drift past N-sigma, evaluator-score regression on a stable dataset. Should we add an `Insight` schema (separate from `Alert` — alerts are configured, insights are detected) + an `AnomalyEngine` that runs alongside `MonitorWorker` and emits `Insight` rows for the cockpit Insights tab + the MCP `list_insights` tool to surface? | **Yes, add it, scoped tight for v1.** (a) `packages/hfao/schema/insights.py` — `Insight(project_id, id, kind, severity, signal_name, baseline_value, current_value, observed_at, evidence_sql, confidence)` with `kind ∈ {trend_shift, threshold_breach_implicit, distribution_drift, calibration_drift, cost_anomaly}`. (b) `packages/hfao/compute/anomaly.py` — pure-Python detectors (rolling-mean Z-score, control-chart Western Electric rules, KS-distance for distribution drift). No new top-level deps; reuses the storage backend's `execute_readonly_sql` for the SQL the engine generates. (c) Cockpit gets one new "Insights" tab (additive, fits inside §10.1 single-file rule); MCP gets `list_insights(project, since, min_severity)` and `get_insight(project, insight_id)`. (d) Insights are append-only; the engine never updates or deletes — the cockpit dedups by `(project_id, signal_name, kind, day)` on render. (e) The §16 entry counts as the §3 layout amendment (adding `compute/anomaly.py` and `schema/insights.py`). | Before any anomaly-detection code lands |
| Q-19 | **Insight routing intelligence (net-new scope beyond §15.2).** Monitor alerts today fan out to user-specified webhook URLs (§8.4 `Monitor.channels`). The Q-19 ask: route insights and alerts to the *right* humans/agents automatically — the on-call engineer for a prod regression, the model owner for a calibration drift, the prompt author for a quality dip on `prompt_label="production"`. Should we add an `InsightRouter` that matches insights against subscription rules (role + ownership + project + kind)? | **Yes, but a thin first version.** (a) `packages/hfao/schema/subscriptions.py` — `Subscription(project_id, id, subscriber_kind, subscriber_id, match_kind, match_signal_name, match_min_severity, channels)` where `subscriber_kind ∈ {role, user, agent}` and `subscriber_id` is e.g. `"role:owner"`, `"user:alice@example.com"`, or `"agent:on-call-bot"`. (b) `packages/hfao/compute/routing.py` — `route(insight_or_alert) -> list[(subscription, channels)]` evaluator. No ML / learning in v1 — this is rule-based matching. The "learning *who* should see *what*" version is explicitly deferred as Q-19-next. (c) Cockpit Settings tab gains a subscriptions sub-section; MCP gets `list_subscriptions(project)` and gated `create_subscription`. (d) Ownership signals come from existing audit-log entries — e.g. the last `create_prompt_version` actor on a prompt is the default "prompt owner" subscriber. No new ownership store. | Before any routing code lands |
| Q-20 | **Pull Stage 2 (counterfactual replay) forward from Week 11 to right-after-v0.5.0.** §15.2 schedules counterfactual replay at Week 11; the schema, the `replay_supported` flag, and the pipeline scaffold all already exist (PR #12). After M2 ships we have slack vs. plan because Q-10a landed faster than budgeted. The differentiator argument: Stage 2 closes "hypothesis → verified cause" for the three Tier-1 frameworks (LangGraph / OpenAI Agents SDK / Claude Agent SDK), which is the most-tweeted-about HFAO claim per the marketing copy in the README. Should we reorder §15.2 to bring Stage 2 immediately after v0.5.0, before Q-18/Q-19 work? | **Yes, reorder.** Cut the `compute/causal/counterfactual.py` + per-framework replay drivers line from Week 11 and place it as the next post-v0.5.0 commit. Rationale: (a) it has no upstream dependencies — every prerequisite (replay-support registry, judge framework, causal pipeline) is on `main`; (b) it converts a public-facing claim ("hypotheses verified by replay where supported") from aspirational to demonstrable, which materially strengthens the v0.5.0 → v1.0.0 narrative; (c) doing it before Q-18 means the Insights tab can include `decisive_error_verified_by_replay` as a first-class insight kind on landing, instead of bolting it on later; (d) the schema `replay_supported` flag is already correct per-framework, so no schema deviation. The Week 11 §15.2 line gets crossed through and the freed slot at Week 11 is the cockpit polish + Helm chart hardening that originally lived there. **Important caveat:** Stage 2 needs to execute user agent code in-process; the AC tests use lightweight stub agents (a deterministic in-process LangGraph fixture), not live LLM calls. | Before the next post-v0.5.0 commit lands |
| Q-21 | SPEC states three unsourced comparative claims against six named competitors: §1.1 line 48 ("Parity with LangSmith / Langfuse / …"), line 49 ("**Exceed them on three pillars commercial competitors cannot easily copy**"), and line 2354 ("This is what makes HFAO **genuinely better than Langfuse/Braintrust** for comparative study"). No head-to-head evaluation has been run — no feature matrix, no methodology, no competitor versions pinned, no source. SPEC.md is public and is hyperlinked from the README sentence that was just corrected to say the opposite, so the two documents now disagree in public. Should these be reframed as design choices rather than measured advantages, matching the corrected README and site wording? | Yes — reframe to scope-and-design-choice language ("targets the same capability surface … no head-to-head evaluation has been run; these are design choices, not measured advantages"), and delete the motive attribution outright. Blocked pending this ruling because §15.3 rule 2 forbids editing SPEC beyond §16 without one. | Before the next public claim review |
...
- **Q-20 resolved (2026-06-03)** — Greenlight the proposed reorder. Stage 2 counterfactual replay implementation lands as the first post-`v0.5.0` commit, ahead of Q-18 and Q-19. *Rationale matches the original entry:* every prerequisite (replay-support registry, judge framework, causal pipeline scaffold) is on `main`; converting "hypothesis" to "verified cause" for Tier-1 frameworks strengthens the public-facing claim from aspirational to demonstrable; doing it before Q-18 lets the Insights tab include `decisive_error_verified_by_replay` as a first-class kind on first arrival. Concrete deliverables: (a) `compute/causal/counterfactual.py` with a `ReplayDriver` Protocol + the Stage-2 pipeline (`stage2_counterfactual`); (b) three replay drivers under `instrumentations/` extras — LangGraph (checkpointer + thread resume), OpenAI Agents SDK (`RunState.from_string` + `Runner.run`), Claude Agent SDK (`resume_from`); (c) `pipeline.PHASE` flipped to `2` so the existing guard activates; (d) AC §8 counterfactual subset using deterministic in-process stub agents per the §16 caveat — no live LLM calls. The Week 11 §15.2 line that originally held this work gets crossed through and the freed slot becomes cockpit polish + Helm chart hardening, as the entry promised.
- **Q-18 resolved (2026-06-04)** — Greenlight as proposed. Lands as the first post-Q-20 commit. *Concrete deliverables:*
  - **(a)** `packages/hfao/schema/insights.py` — `Insight` Struct with `kind ∈ {trend_shift, threshold_breach_implicit, distribution_drift, calibration_drift, cost_anomaly, decisive_error_verified_by_replay}`. The replay-verified kind is added per the Q-20 promise: every Stage-2 `COUNTERFACTUAL_REPLAY` edge surfaces as an Insight so the cockpit + MCP route it through the same insight pipeline as anomaly detections.
  - **(b)** `packages/hfao/compute/anomaly.py` — pure-Python detectors. `RollingMeanZScore` (configurable window + Z threshold, two-sided), `WesternElectricRules` (the standard 8-of-1 / 2-of-3 / 4-of-5 / 8-in-a-row control-chart variants), `KSDistance` (one-sample two-sided Kolmogorov–Smirnov vs. a baseline distribution), `CalibrationDrift` (judge-vs-human RMSE rolling), and a `ReplayVerifiedScanner` that lifts Q-20 edges into Insights. The `AnomalyEngine` runs alongside `MonitorWorker`, reading via `StorageBackend.execute_readonly_sql` so no new SQL escapes `storage/`.
  - **(c)** Control plane gains an `insights` table with `(project_id, signal_name, kind, observed_at)` indexing; CRUD is append-only — engine never updates or deletes.
  - **(d)** Cockpit gets one new **Insights** tab (additive to `apps/cockpit/cockpit.py` per §10.1 single-file rule). Dedup-on-render groups by `(project_id, signal_name, kind, observed_at::date)`.
